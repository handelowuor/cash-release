...
function AuthCheck({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!isLoading && authAttempted && !isAuthenticated) {
      console.log("Accountabilities: Not authenticated, redirecting to login", {
        user,
        authAttempted,
      });
      router.push("/login");
    } else if (!isLoading && isAuthenticated) {
      console.log("Accountabilities: User is authenticated", {
        hasUser: !!user,
        hasToken: !!user?.token,
...
export default function AccountabilitiesLayout({
  children,
}: {
